...
namespace Business.Handlers.RotaDetayis.Commands
{
    ...
    public class AddPhotoCommad : IRequest<IResult>
    {
        ...
        public class AddPhotoCommadHandler : IRequestHandler<AddPhotoCommad, IResult>
        {
            ...
            {
                var result = await _mediator.Send(new GetRotaDetayiQuery { RotaDetayiId = request.RotaDetayiId });
                if (request.File.Length > 0)
                {
                    string folderPath = Path.Combine(_hostingEnvironment.WebRootPath, "uploads/rotadetayi");
                    ...
                    result.Data.Foto = "/uploads/rotadetayi/" + request.File.FileName;
                    /*myClass.Photo = "/uploads/" + file.FileName; */
                    var upResult = await _mediator.Send(new UpdateRotaDetayiCommand()
                    {
                        RotaDetayiId = result.Data.RotaDetayiId,
